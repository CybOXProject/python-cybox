...
class SocketOptions(cybox.Entity):
    ...
    ip_multicast_loop = cybox.TypedField("IP_MULTICAST_LOOP", xs.boolean)
    ip_tos = cybox.TypedField("IP_TOS", String)
    so_broadcast = cybox.TypedField("SO_BROADCAST", xs.boolean)
    so_conditional_accept = cybox.TypedField("SO_CONDITIONAL_ACCEPT", xs.boolean)
    so_keepalive = cybox.TypedField("SO_KEEPALIVE", xs.boolean)
    so_dontroute = cybox.TypedField("SO_DONTROUTE", xs.boolean)
    so_linger = cybox.TypedField("SO_LINGER", UnsignedInteger)
    so_dontlinger = cybox.TypedField("SO_DONTLINGER", xs.boolean)
    so_oobinline = cybox.TypedField("SO_OOBINLINE", xs.boolean)
    so_rcvbuf = cybox.TypedField("SO_RCVBUF", UnsignedInteger)
    so_group_priority = cybox.TypedField("SO_GROUP_PRIORITY", UnsignedInteger)
    so_reuseaddr = cybox.TypedField("SO_REUSEADDR", xs.boolean)
    so_debug = cybox.TypedField("SO_DEBUG", xs.boolean)
    ...
    tcp_nodelay = cybox.TypedField("TCP_NODELAY", xs.boolean)
...
class NetworkSocket(ObjectProperties):
    ...
    is_blocking = cybox.TypedField("is_blocking", xs.boolean)
    is_listening = cybox.TypedField("is_listening", xs.boolean)
