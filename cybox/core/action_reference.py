...
class ActionReference(cybox.Entity):
    ...
    def to_obj(self):
        ...
        if self.action_id is not None : action_reference_obj.set_action_id(self.action_id)
        return action_reference_obj

    def to_dict(self):
        action_reference_dict = {}
        if self.action_id is not None : action_reference_dict['action_id'] = self.action_id
